...
const getFieldTransition = (delay = 0.3, duration = 1, baseDelay = 0) => ({
  duration,
  delay: baseDelay + delay,
  ease: entranceEase,
});

const MobileProjectCard = React.memo(function MobileProjectCard({
  project,
  baseDelay = 0,
}: {
  project: Project;
      ...
      <CardContent className="pt-0">
        <div className="mb-2">
          {isMobileTablet ? (
            ...
            <>
              <h4 className="text-xs sm:text-sm font-semibold text-foreground/90 mb-1.5">
                Technologies
              </h4>
              <div className="flex flex-wrap gap-1.5 max-w-[22.5rem]">
...
export const PortfolioSection = memo(function PortfolioSection({
  ...
  const isMobile =
    typeof isMobileOverride === "boolean" ? isMobileOverride : hookIsMobile;

  const maxIdx = visibleProjects.length - 1;
  const animMs = 300;

  const safeStep = useCallback(
    (dir: 1 | -1) => {
  ...
  const sectionClass = useMemo(
    () =>
      "w-full h-full flex flex-col items-center justify-center pt-8 pb-24 sm:py-12 md:pt-16 md:pb-24 lg:pt-16 lg:pb-24 scroll-mt-16",
    []
  );

  const baseDelay =
    typeof contentEnterDelay === "number" ? contentEnterDelay : 0;
  ...
  return (
    <section ref={sectionRef} id="portfolio" className={sectionClass}>
      <div className="w-full px-4 sm:px-8 md:px-8 lg:px-12 xl:px-16 2xl:px-20">
        <m.div
          className="flex flex-col items-center justify-center space-y-1 text-center mb-8 sm:mb-10 max-w-6xl mx-auto ipadpro:max-w-7xl"
        ...
        >
          <m.div style={{ overflow: "hidden" }}>
            <m.h2
              ...
              viewport={
                !parentVisible ? { once: true, amount: 0.35 } : undefined
              }
        ...
        {isMobile ? (
          ...
          <div className="relative w-full max-w-7xl mx-auto">
            <div
              className="relative"
              style={{ height: "clamp(420px, 55vh, 550px)" }}
            >
              {/* Main Container */}
                ...
                {visibleProjects.map((project, i) => (
                  ...
                  >
                    <SplitViewCard
                      ...
                      baseDelay={baseDelay}
                      contentCanAnimate={Boolean(contentCanAnimate)}
                      parentContentVisible={parentVisible}
                    />
                  </div>
                ))}
              </div>
                      ...
                      <div
                        className={`w-2 h-2 rounded-full transition-all duration-300 ${
                          i === activeIdx
                            ...
                            : "bg-foreground/20 group-hover:bg-foreground/40 group-hover:scale-110"
                        }`}
                      />
                    </button>
                  ))}
